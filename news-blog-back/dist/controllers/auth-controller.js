...
var AuthController = /** @class */ (function () {
    ...
    AuthController.prototype.login = function (req, res) {
        return __awaiter(this, void 0, void 0, function () {
            var _a, login, password, userRepository, user, error_1, token, response;
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        _a = req.body, login = _a.login, password = _a.password;
                        if (!(login && password)) {
                            return [2 /*return*/, res.status(400).send('login or password is empty')];
                        }
                        userRepository = typeorm_1.getRepository(Users_1.Users);
                        _b.label = 1;
                    case 1:
                        _b.trys.push([1, 3, , 4]);
                        return [4 /*yield*/, userRepository.findOneOrFail({ where: { login: login } })];
                    case 2:
                        user = _b.sent();
                        return [3 /*break*/, 4];
                    case 3:
                    ...
                    case 4:
                        token = jwt.sign({ id: user.id, login: user.login }, process.env.JWT_SECRET, { expiresIn: "24h" });
                        response = {
                            'id': user.id,
                            'login': user.login,
                            'email': user.email,
